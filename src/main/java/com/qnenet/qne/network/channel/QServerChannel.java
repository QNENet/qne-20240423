...
public class QServerChannel {
    ...
    public QServerChannel(QNode node, byte[] keyPairClone) throws NoSuchAlgorithmException, ShortBufferException {
        this.node = node;
    ...
    private void startHandshake(byte[] keyPairClone) throws NoSuchAlgorithmException, ShortBufferException {
        handshake = new HandshakeState("Noise_XX_25519_ChaChaPoly_BLAKE2b", HandshakeState.RESPONDER);
    ...
    private void readHandshakeMessage(QNEPacket qnePacket) throws ShortBufferException, BadPaddingException {
        ...
        if (payloadSize > 0) {
            ...
            // QChannelInfo reeivedChannelInfo = (QChannelInfo)
            // qobjs.objFromBytes(payloadBytes);
